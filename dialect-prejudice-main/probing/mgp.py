...
def main():
    ...
    with torch.no_grad():
        ...
        if args.calibrate:
            with open(f"{helpers.PROBS_PATH}{os.path.sep}{model_tag}_{args.variable}_{args.attribute}_cal.p", "wb") as f:
                pickle.dump(prompt_results, f)
        else:
            with open(f"{helpers.PROBS_PATH}{os.path.sep}{model_tag}_{args.variable}_{args.attribute}.p", "wb") as f:
                pickle.dump(prompt_results, f)
